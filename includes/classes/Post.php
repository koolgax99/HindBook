...
class Post
{
	...
	public function submitPost($body, $user_to, $imageName, $type)
	{
		...
		if ($check_empty != "" || $imageName != "") {
			...
			//insert post 
			$query = mysqli_query($this->con, "INSERT INTO posts VALUES (NULL, '$body', '$added_by', '$user_to', '$date_added', 'no', 'no', '0', '$imageName', '$type')");
			...
			while ($row = mysqli_fetch_array($data_query)) {
				...
				if ($user_logged_obj->isFriend($added_by)) {
					...
					if ($userLoggedIn == $added_by)
						$delete_button = "<button class='btn btn-danger' id='post$id'>Delete</button>";
					else
						$delete_button = "";
...
?>
	...
	public function loadShopPosts($data, $limit)
	{
		...
		$str = ""; //String to return 
		$data_query = mysqli_query($this->con, "SELECT * FROM posts WHERE deleted='no' AND type='shop' ORDER BY id DESC");
